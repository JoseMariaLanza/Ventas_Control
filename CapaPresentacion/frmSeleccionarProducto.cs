...
{
    public partial class frmSeleccionarProducto : MaterialForm
    {
        ...
        private void frmSeleccionarProducto_Load(object sender, EventArgs e)
        {
            ...
            txtBuscar.Hint = "Buscar por nombre";
            txtBuscarCategoria.Hint = "Buscar por categoría";
            txtBuscarCodigo.Hint = "Buscar por código";
        }

        #region INSTANCIACIÓN
        private static frmSeleccionarProducto _Instancia = null;

        public static frmSeleccionarProducto Instancia
        {
            get
            {
                return _Instancia;
        ...
        public static frmSeleccionarProducto GetInstancia()
        {
            if (Instancia == null)
            {
                Instancia = new frmSeleccionarProducto();
            }
            return Instancia;
        }
        ...
        public void Mostrar()
        {
            dgvListado.DataSource = NegocioProducto.Mostrar();
            NombreColumnas();
            OcultarColumnas();
        }
        #endregion
        ...
        {
            dgvListado.DataSource = NegocioProducto.BuscarNombre(txtBuscar.Text);
            OcultarColumnas();
        }
        ...
        {
            dgvListado.DataSource = NegocioProducto.BuscarProductoCategoria(txtBuscar.Text, txtBuscarCategoria.Text);
            OcultarColumnas();
        }
        ...
        {
            dgvListado.DataSource = NegocioProducto.BuscarCategoria(txtBuscarCategoria.Text);
            OcultarColumnas();
        }
        ...
        {
            dgvListado.DataSource = NegocioProducto.BuscarCodigo(txtBuscarCodigo.Text);
            OcultarColumnas();
        }
        ...
        {
            IdArticulo = Convert.ToInt32(dgvListado.CurrentRow.Cells["idproducto"].Value);
            Codigo = Convert.ToString(dgvListado.CurrentRow.Cells["codigo"].Value);
            Articulo = Convert.ToString(dgvListado.CurrentRow.Cells["nombre"].Value);
            IdCategoria = Convert.ToInt32(dgvListado.CurrentRow.Cells["idcategoria"].Value);
            Precio_Compra = Convert.ToDecimal(dgvListado.CurrentRow.Cells["precio_compra"].Value);
            Precio_Venta = Convert.ToDecimal(dgvListado.CurrentRow.Cells["precio_venta"].Value);
            ...
            Ruta_Imagen = Convert.ToString(dgvListado.CurrentRow.Cells["ruta_imagen"].Value);
        }
        #endregion
